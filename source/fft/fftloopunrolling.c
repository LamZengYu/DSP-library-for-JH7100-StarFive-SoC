...
void fft(float data_re[], float data_im[], const int N)
{
  ...
  {
    if(target>position) {
      float temp_re = data_re[target];
      float temp_im = data_im[target];
  ...
  float step_d, twiddle_re, twiddle_im, product_re, product_im, x, y;
  ...
  for(unsigned int step=1; step<N; step <<=1)
  {
    ...
    for(unsigned int group=0; group<step; group++)
    {
      ...
      {
        product_re = twiddle_re*x-twiddle_im*y;
        product_im = twiddle_im*x+twiddle_re*y;
...
void running_time(float data_re[], float data_im[], const int N) {
    ...
    start = clock();
    for(int i=0;i<10000;i++) fft(data_re, data_im, N);
    stop = clock();
    cpu_time_used = ((double) (stop - start)) / CLOCKS_PER_SEC;
    printf("Average time per fft %fms", cpu_time_used/10);
}
